...
export default function CourseLessons() {
  ...
  const getLessonStatusColor = (lessonId: string) => {
    ...
    if (lessonProgress?.completed_at) return '#10b981'; // Green for completed
    if (lessonProgress && !lessonProgress.completed_at) return '#f59e0b'; // Orange for in progress
    return '#6b7280'; // Gray for not started
  };

  const getLessonStatusText = (lessonId: string) => {
    const lessonProgress = progress[lessonId];
    if (lessonProgress?.completed_at) return 'Completed';
    if (lessonProgress && !lessonProgress.completed_at) return 'In Progress';
    return 'Not Started';
  };
